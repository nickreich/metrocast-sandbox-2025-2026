"""GBQR model using MCHub + ILINet + Weather data.

This model uses:
- MCHub target data (primary source)
- ILINet outpatient surveillance (supplementary)
- Weather data features (temperature, humidity, precipitation)
  - Raw values
  - 1-week and 2-week lags
  - 3-week trailing rolling average

NOTE: Requires extended weather data history (10+ years) to match ILINet history.
"""
...
def main(today_date: str, short_run: bool, use_local_mchub: bool,
    ...
    """Generate GBQR forecasts using MCHub + ILINet + Weather data."""
    ...
    # Model configuration - MCHub + ILINet + Weather features
    model_config = ModelConfig(
        model_name="gbqr_ili_weather",

        # ILINet supplementary source
        use_ilinet=True,
    ...
    # Print configuration
    click.echo("Running GBQR model: gbqr_ili_weather")
    click.echo(f"  Reference date: {reference_date}")
    click.echo(f"  Num bags: {model_config.num_bags}")
    click.echo(f"  Data sources: MCHub + ILINet + Weather")
